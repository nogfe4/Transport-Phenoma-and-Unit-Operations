...
tubulação), o script calcula:
    ...
    - Fator de atrito (Hagen-Poiseuille para regime laminar;
      aproximação de Swamee-Jain/Colebrook para regime turbulento)
...
# Denominador:
logar = mt.log10((rugosidade_relativa / 3.7) + (5.74 / (Re ** 0.9)))
...
linha_dado("Área da Tubulação", f"{A:.6f}", "m²")
sep_tabela()
linha_dado("Velocidade", f"{v:.4f}", "m/s")
sep_tabela()
linha_dado("Vazão Mássica", f"{w:.4f}", "kg/s")
...
print()
borda_tabela(topo=True)
linha_dado("Número de Reynolds", f"{Re:.2f}")
sep_tabela()
linha_dado("Fator de Atrito", f"{f:.4f}")
sep_tabela()
linha_dado("Perda de Carga (hf)", f"{hf:.4f}", "m")
borda_tabela(topo=False)
linha_dado("Queda de Pressão (ΔP)", f"{delta_P_kPa:.4f}", "kPa")
borda_tabela(topo=False)
print()
